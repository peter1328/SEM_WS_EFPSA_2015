...
# Install package needed for analysis
#install.packages("foreign", "semPlot", "lavaan", "psych", "semTools")
...
######--- The regression coefficient is exactly the same
...
# Configurak invariance
# Between countries

# Prepare data file country information
risk$Country[is.na(risk$Country)] = "The Netherlands"

# 6.1 Investigate full invariance level
measurementInvariance(model5,
                      data=risk,
                      group = "Country")

# 6.2 Model 6:
# Configural invariance model
fit_model6_1 <- cfa(model5,
                    data=risk,
                    missing="ml",
                    group = "Country")
summary(fit_model6_1,
        fit.measures = TRUE,
        standardized = TRUE)
...
fit_model6_2 <- cfa(model5,
                    data=risk,
                    missing="ml",
                    group = "Country",
                    group.equal = c("loadings",
                                    "intercepts"),
                    group.partial = c("Risk=~ZMar1",
                                      "ZMar1~1"))
summary(fit_model6_2,
        fit.measures = TRUE,
        standardized = TRUE)

# Compare model fit between models 6_1 and 6_2
anova(fit_model6_2, fit_model6_1)

###--- Non-significant difference;
###--- We have invariance!
###--- The same construct is measured
###--- In Austria and in the Netherlands
...
# 11.2 Model 11:
# Configural invariance model
fit_model11_1 <- cfa(model11,
                    data=risk,
                    missing="ml"
                    )
summary(fit_model11_1,
        fit.measures = TRUE,
        standardized = TRUE)
# Draw SEM model figure
semPaths(fit_model11_1,
         "model", # "classic" style
         "estimates",
         #"std", # Show standardized parameter estimates
         #style = "lisrel", # LISREL Style: Errors single headed
